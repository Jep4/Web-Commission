 ...
 **************************************************************************/
	...
	if($page > $show_page_num) {
		...
		$print_page.="<a onfocus=blur() href='$PHP_SELF?id=$id&page=1&select_arrange=$select_arrange&desc=$desc&category=$category&sn=$sn&ss=$ss&sc=$sc&keyword=$keyword&sn1=$sn1&divpage=$divpage'>1</a> ...";
	...
	while($i+$start_page<=$total_page && $i<=$show_page_num) {
		...
		if($page == $move_page) $print_page.=" <span class='color'>HERE</span>$slash";
		else $print_page.= "<a onfocus=blur() href='$PHP_SELF?id=$id&page=$move_page&select_arrange=$select_arrange&desc=$desc&category=$category&sn=$sn&ss=$ss&sc=$sc&keyword=$keyword&sn1=$sn1&divpage=$divpage'>$move_page </a>$slash";
		$i++;
	}
	...
	if($total_page>$move_page) {
		...
		$print_page.="... <a onfocus=blur() href='$PHP_SELF?id=$id&page=$total_page&select_arrange=$select_arrange&desc=$desc&category=$category&sn=$sn&ss=$ss&sc=$sc&keyword=$keyword&sn1=$sn1&divpage=$divpage'>$total_page</a>";
...
<table border=0 cellspacing=0 cellpadding=0 width=<?=$width?>>
	<tr>
		<td align="left" class="small6" style="padding-top:4px;"><?=$a_list?>LIST</a>
		<?=$a_delete_all?> : DELETE</a></td>
		<td align="right" class="small6" style="padding-top:4px;" valign=top><?=$a_prev_page?>◀&nbsp;</a> <?=$print_page?>  <?=$a_next_page?>&nbsp;▶</a></td>
		</form>
	</tr>
</table>
